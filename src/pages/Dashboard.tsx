...
import { formatTokenAmount, formatRelativeTime, formatAddress } from '../utils/formatters';
import './Dashboard.css';

const Dashboard: React.FC = () => {
  const { claimsData, balancesData, isLoading, error, lastUpdated, refreshData } = useCSVData();
    ...
    <div className="dashboard-container">
      ...
      <div className="stats-grid">
        ...
        <StatCard
          ...
          value={formatTokenAmount(BigInt(stats.totalUnclaimedAmount), 18, 'ASTR')}
          isLoading={isLoading}
          icon={
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="contracts-table-container">
              ...
              {isLoading ? (
                ...
                Array.from({ length: 5 }).map((_, index) => (
                  <tr key={index} className="loading-row">
                    <td><div className="loading-placeholder"></div></td>
                    <td><div className="loading-placeholder"></div></td>
                    <td><div className="loading-placeholder"></div></td>
                  </tr>
                ))
                ...
                balancesData.map((balance) => {
                  // Find the corresponding claim entry to get the user address
                  const claimEntry = claimsData.find(
                    (claim) => claim.vestingWallet.toLowerCase() === balance.vestingWallet.toLowerCase()
                  );
                  ...
                  return (
                    <tr key={balance.vestingWallet}>
                      ...
                      <td className="address-text">
                        {claimEntry ? formatAddress(claimEntry.account) : 'Unknown'}
                      </td>
                      <td>
                        {formatTokenAmount(BigInt(balance.amount), 18, 'ASTR')}
                      </td>
                    </tr>
                  );
                })
        ...
        <div className="contracts-table-footer">
          <div className="contracts-table-info">
            Showing {balancesData.length} of {balancesData.length} active contracts
          </div>
        </div>
      </div>
    </div>
